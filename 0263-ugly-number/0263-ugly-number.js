...
var isUgly = function(n) {
    ...
    let divisors = [2, 3, 5];
    let curIndex = 0;
    while (divisors[curIndex] && n != 1){
        if (n%divisors[curIndex] == 0)
        {
            n /= divisors[curIndex];
        }
        else{
    // console.log(n, curIndex);
            ++curIndex;
        }
    }
    // console.log(n, curIndex);
...
//old solution that was way too overly complicated
var noPrimesPast5 = (n) => {
    let skips = {'2': 2};
    let primes = {};
    let factorsHigh = [];
    // let iterations = 0;
    var i = 2;
    while (i <= n){ // only need odd numbers, except 2, so we start at 2, then 3 and odds from there
        let skip = skips[i];
        // console.log("current", i, "skip", skip, "skips", skips);
        if (skip){ //don't need it since it's a multiple of a prime
        ...
        let high = (n/i);
        if (n % i == 0 && i < high && high % 2 != 0){ // odd upper factor
